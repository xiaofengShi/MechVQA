...
def format_reward_r1(response: str) -> float:
    pattern = re.compile(r"ground_truth\s*<answer>.*?</answer>", re.DOTALL)
...
1) logic: internal coherence and continuity of reasoning, and whether the conclusion is supported. Missing key steps or contradictions should lower the score.
2) professionalism: use of mechanical drawing / engineering drawing terminology and conventions (e.g., views, sections, dimensions, tolerances, datums, fits, GD&T, surface roughness, assembly relations). Misused terms or overly casual phrasing should lower the score.
3) conciseness: include only what is needed to answer the question. Off-topic, verbose, repetitive content should lower the score.
...
def judge_scores_llm(
    ...
    if isinstance(obj, dict):
        for k in ["logic", "professionalism", "conciseness"]:
...
def _compute_single_score(
    ...
    # accuracy 改为 F1（response 的 answer vs 标准答案的 answer）
    answer = extract_answer(response)
    gt_answer = extract_answer(ground_truth)
    accuracy_score = float(_f1_score(answer, gt_answer))
    ...
    if include_reasons:
        result.update(
            {
                "reason_accuracy": "f1(answer, ground_truth_answer)",
...
def _f1_score(a: str, b: str) -> float:
    """
    基于 token 的 F1。
    """
    a_tokens = _tokenize_for_f1(a)
    b_tokens = _tokenize_for_f1(b)
