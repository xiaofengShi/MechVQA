...
# enable_thinking 由环境变量 ENABLE_THINKING 控制（默认 true 不影响别的项目；
# 当前 answer-only 训练设 ENABLE_THINKING=false 禁 Qwen3 thinking）
ENABLE_THINKING = os.environ.get("ENABLE_THINKING", "true").lower() not in ("false", "0", "no", "")
...
class RLHFDataset(Dataset):
    ...
    def _filter_overlong_prompts(self, example: dict[str, Any]) -> bool:
        ...
        if self.image_key in example and len(example[self.image_key]) > 0:
            prompt = self.processor.apply_chat_template(messages, add_generation_prompt=True, enable_thinking=ENABLE_THINKING, tokenize=False)
            ...
            input_ids = self.tokenizer.apply_chat_template(messages, add_generation_prompt=True, enable_thinking=ENABLE_THINKING)
    ...
    def __getitem__(self, index):
        ...
        if self.image_key in example and len(example[self.image_key]) > 0:
            ...
            prompt = self.tokenizer.apply_chat_template(messages, add_generation_prompt=True, enable_thinking=ENABLE_THINKING, tokenize=False)
